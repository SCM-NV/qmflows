...
def PES_scan(package, settings, molecule, scan, job_name="PESscan"):
    ...
    def lt_job(settings, job_name):
        if isinstance(package, list):
            ...
            optimized_mol = package[0](templates.geometry.overlay(settings),
                                       molecule, job_name=name).molecule
            result = package[1](templates.singlepoint.overlay(settings),
                                optimized_mol, job_name=job_name)
        else:
            result = package(templates.geometry.overlay(settings), molecule,
                             job_name=job_name)
        return result
    ...
    def perform_scan(scan, settings, job_name):
        for step in range(scan['surface']['nsteps']):
            ...
            const_settings = get_constraint_settings(constraint, start,
                                                     stepsize, step)
            new_settings = settings.overlay(const_settings)
            name = job_name + "_" + str(step)
            if 'scan' in scan:
                perform_scan(scan['scan'], new_settings, name)
            else:
                lt_jobs.append(lt_job(new_settings, name))
