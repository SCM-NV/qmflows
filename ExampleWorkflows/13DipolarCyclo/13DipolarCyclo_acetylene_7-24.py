...
def calc_reactant(name):
    ...
    reactant = adf(templates.geometry.overlay(settings),
                   dftb(templates.geometry.overlay(settings), mol, job_name=name+"_reactant_DFTB").molecule, job_name=name+"_reactant")
    return reactant
...
def calc_productAndTS(name):
    ...
    product = adf(templates.geometry.overlay(settings),
                  dftb(templates.geometry.overlay(settings), mol, job_name=name+"_product_DFTB").molecule, job_name=name+"_product")
    ...
    PES = PES_scan([dftb,adf], settings, product.molecule, scan, job_name=name+"_PES")
                
    # get the object presenting the molecule with the maximum energy calculated from the scan
    apprTS = select_max(PES, 'energy')
    
    DFTB_freq = dftb(templates.freq.overlay(settings), apprTS.molecule, job_name=name+"_DFTBfreq")
    
    t = Settings()
    t.specific.adf.geometry.inithess = DFTB_freq.archive.path
    
    # Run the TS optimization, using the default TS template
    TS = adf(templates.ts.overlay(settings).overlay(t), DFTB_freq.molecule, job_name=name+"_TS")

    adf_freq = adf(templates.freq.overlay(settings), TS.molecule, job_name=name+"_freq")

    return product,adf_freq

# systematically generate list of names consisting of the 3 atoms that vary in the series of reactions
# e.g. "CNC", "CNN", etc. 
reaction_names = set()
for a1 in ('C','N','O'):
    for a2 in ('N','O','S'):
...
E_acetylene_job = adf(templates.geometry.overlay(settings), acetylene, job_name="acetylene").energy

# Actual execution of the jobs
E_acetylene, results = run(gather(E_acetylene_job, gather(*job_list)), n_processes = 1)
...
for smiles in table:
    print("{0:20s} {1:7.1f} {2:7.1f} {3:7.2f} {4:7.2f}".format(smiles, *table[smiles]))
